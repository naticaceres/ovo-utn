...
export const authApi = {
  async login(payload: AuthPayload): Promise<User | null> {
    const { data } = await api.get(`/users`, { params: { email: payload.email } })
...
export const questionnaireApi = {
  ...
  async submitAnswers(userId: number, answers: Answer[]): Promise<{ id: number }> {
    const { data } = await api.post('/answers', { userId, answers, createdAt: new Date().toISOString() })
    return data
  },
}

export type Recommendation = { id: number; career: string; university: string; link: string }
