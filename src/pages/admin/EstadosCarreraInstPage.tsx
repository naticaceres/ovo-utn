...
export default function EstadosCarreraInstPage() {
  ...
  const load = React.useCallback(async () => {
    setLoading(true);
    try {
      const data = await listCareerStates();
      setItems(Array.isArray(data) ? data : []);
    } catch {
      setError('No se pudieron cargar los estados de carrera institución');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const openEdit = (it: Item) => {
    ...
    if (it.fechaFin && it.fechaFin !== 'NULL' && it.fechaFin.trim() !== '') {
      // Manejar formato "Mon, 03 Nov 2025 17:56:45 GMT" o "YYYY-MM-DD HH:MM:SS"
      try {
  ...
  const save = async () => {
    ...
    try {
      ...
      if (editing) {
        if (reactivar) {
          payload.fechaFin = null; // Reactivar el estado
        } else if (fechaFin) {
      ...
      if (editing) {
        await updateCareerState(editing.id, payload);
      } else {
        await createCareerState({ nombre: name });
      }
      setShowModal(false);
      load();
    } catch {
  ...
  const remove = async () => {
    ...
    setDeleting(true);
    try {
      await deactivateCareerState(itemToDelete.id, itemToDelete.nombre);
  ...
  // Función para filtrar estados de carrera institución
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.header}>
        <h1>Estado de carrera institución</h1>
      ...
      {showModal && (
        <div className={styles.modalBackdrop}>
          <div className={styles.modal}>
            <h3>
              {editing
                ? 'Editar estado de carrera institución'
                : 'Agregar estado de carrera institución'}
            </h3>
            <Input
              label='Nombre'
              fullWidth
      ...
      <ConfirmDialog
        ...
        message={
          <>
            ...
            <p>
              Esta acción afectará a todas las carreras que usen este estado.
            </p>
          </>
        }
