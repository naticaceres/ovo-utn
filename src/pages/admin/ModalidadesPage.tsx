...
export default function ModalidadesPage() {
  ...
  const load = React.useCallback(async () => {
    ...
    try {
      ...
      setError('No se pudieron cargar las modalidades de carrera');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const save = async () => {
    ...
    try {
      if (editing) {
        await updateModality(editing.id, { nombre: name });
      } else {
        await createModality({ nombre: name });
      }
      setShowModal(false);
      await load();
    } catch (err: unknown) {
  ...
  const remove = async () => {
    ...
    setDeleting(true);
    try {
      await deactivateModality(itemToDelete.id, itemToDelete.nombre);
    ...
    <div className={styles.container}>
      ...
      >
        ...
        <div style={{ fontSize: '14px', color: '#666' }}>
          ...
          <strong>{items.length}</strong> modalidades
        </div>
      </div>
      ...
      {showModal && (
        <div className={styles.modalBackdrop}>
          <div className={styles.modal}>
            <h3>
              {editing
                ? `Editar modalidad de carrera`
                : `Agregar modalidad de carrera`}
            </h3>
            <Input
              label='Nombre'
              fullWidth
      ...
      <ConfirmDialog
        ...
        message={
          <>
            ...
            <p>Esta acción afectará a todas las carreras con esta modalidad.</p>
          </>
        }
        confirmText='Sí, Eliminar Modalidad'
        cancelText='Cancelar'
        variant='danger'
        isLoading={deleting}
      />
